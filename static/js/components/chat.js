...
function getBotResponse(text) {
    let botResponse = `<img class="botAvatar" src="./static/img/sara_avatar.png"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;
    return botResponse;
}

// speak the response back to the user
function speakUtterance(text) {
    console.log("before speak: ", text)
            ...
            for (let i = 0; i < response.length; i += 1) {
                ...
                if (Object.hasOwnProperty.call(response[i], "text")) {
                    if (response[i].text != null) {
                        ...
                        let botResponse;
                        let responseText = response[i].text
                        ...
                        $(botResponse).appendTo(".chats").hide().fadeIn(1000);
                        // speakUtterance(botResponse.messages)
                        speakUtterance(responseText);
                    }
                }
...
/** funciton to auto open chat window */
...
$(".talk").on("click", (e) => {
    ...
    recognition.onstart = function() {
        console.log('voice activated, you are speaking');
        $("#userInput").prop("disabled", true);
    }
...
video.addEventListener('play', () => {
    ...
    if (detections.length !== 0) {
        let experssions = detections[0].expressions;
        let curVal = "";
        let curMax = -1;
        let threshold = 0.6;
        for (let [key, value] of Object.entries(experssions)) {
            if (curMax < value){
                curMax = value;
                curVal = key;
            }
          }
        if (curMax > threshold && curVal !== "neutral"){
            console.log(curVal);
            const f_expression = [ 'happy', 'sad', 'surprised', 'angry' ];
            if (f_expression.includes(curVal)){
                if (firstTimeVid){
                    openChatWindow();
                    firstTimeVid = false;
                }
